...
def parting():
    ...
    while k < len(glucose):
        ...
        if switch >= 2 and count > 50:
            ...
            if (positive_trend >= threshold and negative_trend >= threshold) or (
                    positive_trend_prev >= threshold and negative_trend >= threshold) or (
                    positive_trend >= threshold and negative_trend_prev >= threshold):
    ...
    for search_item in r_list:
        for compare_item in r_list[r_list.index(search_item) + 1:]:
            ...
            if isclose_values != 0:
                ...
                ripple_connections[r_list.index(search_item)].append(
                    (percent_search_value, r_list.index(search_item), r_list.index(compare_item)))
